...
namespace SMSProcessor
{
    ...
    public partial class frmMain : Form
    {
        ...
        {
            WriteLogFile.WriteLogSMS(string.Format("Starting SMS"));
            // SmsConfig();
            ...
            //var response = SendVoiceSMS_V1("74039", "01764010666", 1000,DateTime.Now);
        }
        ...
        private void schedule_Timer_Callback()
        {
            ...
            vsTimer.Interval = 2000;
            vsTimer.Enabled = true;
            vsTimer.Elapsed += VsTimer_Tick;
            vsTimer.Start();


            //vsTimer2 = new System.Timers.Timer();
            //vsTimer2.Interval = 2000;
            //vsTimer2.Enabled = true;
            //vsTimer2.Elapsed += VsTimer2_Tick;
            //vsTimer2.Start();
        ...
        private void VsTimer_Tick(object sender, ElapsedEventArgs e)
        {
            try
            {
                ...
                if (today.TimeOfDay >= new TimeSpan(10, 00, 00) && today.TimeOfDay < new TimeSpan(23, 59, 59))
                {
            ...
            {
                WriteLogFile.WriteLogVoice(ex.Message);
        ...
        //private void VsTimer2_Tick(object sender, ElapsedEventArgs e)
        //{
        //    try
        //    {
        //        vsTimer2.Stop();
        ...
        //    catch (Exception ex)
        //    {
        //        WriteLogFile.WriteLogVoice2(ex.Message);
        ...
        private void SmsConfig()
        {
            ...
            catch (Exception ex)
            {
                ...
                WriteLogFile.WriteLogSMS(string.Format(ex.Message));
        ...
        //{
        //    DataTable dt = db.GetDataByProc("sp_GetOtpSMS");
        //    OtpSMSProcess(dt);
        //}
        private void dailySMSProcessor()
        {
            DBUtility db = new DBUtility();
            DataTable dt = db.GetDataByProc("sp_getsmsinfo");
            SMSProcess(dt);
        ...
        private void dailyVoiceNotificationForDue()
        {
            ...
            DataTable dt = new DBUtility().GetDataByProc(ht, "Sp_GetPending_BillDuesVoiceNotification");
            //DataTable dt = new DBUtility().GetDataBySQLString("Select Id,Mobile,CustomerID,BillAmount,DueDate from SMS_Schedule where  SMSType=20   and CustomerId='74039'");

            ht.Clear();
            //Id,Mobile,CustomerID,BillAmount,DueDate
            ...
            foreach (DataRow dr in dt.Rows)
            {
                try
                {
                    ...
                    billAmt = Convert.ToInt32(dr["BillAmount"].ToString().Trim());
                    dueDate = Convert.ToDateTime(dr["DueDate"].ToString().Trim());
                    ...
                    if (IsValidPhoneNumber(Mobile))
                    {
                        ...
                        if (Mobile.Length == 11)
                        {
                            ...
                            var response = SendVoiceSMS_V1(CustomerID, Mobile, billAmt, dueDate);
                            if (response != null && response.request_status == "1")
                            {
                                ...
                                var logMsg = string.Format("{0}({2})==>Send==> {1}: Duration: {3} Seconds", CustomerID, response.message.ToString(), Mobile, duration);
                ...
                {
                    WriteLogFile.WriteLogVoice(CustomerID + ":: " + ex.Message.ToString());
                    UpdateVoiceStatus(SchedulerID, 2);
        ...
        //private void OtpSMSProcess(DataTable dt)
        //{
        ...
        //                        var logMsg = string.Format("{0}({2})==>Send==> {1}: Duration: {3} Seconds", Mobile, response.message.ToString(), Mobile, duration);
